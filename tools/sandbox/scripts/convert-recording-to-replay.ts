...
const input =
  process.argv[2] || "tools/sandbox/scenarios/recorded/2026-02-17T01-17-49-llm-simulation.md";
...
const AGENT_MAP: Record<string, string> = {
  ...
  "Perch Perkins": "perch-perkins",
  "Barnacle Boy": "barnacle-boy",
  "Squilliam Fancyson": "squilliam-fancyson",
  "Patrick Star": "patrick-star",
  Gary: "gary",
  Plankton: "plankton",
  "Plankton Jr.": "plankton-jr",
  "Plankton Jr": "plankton-jr",
...
function resolveTarget(target: string): string {
  for (const [key, val] of Object.entries(AGENT_MAP)) {
    if (
      ...
      key.toLowerCase().startsWith(target.toLowerCase())
    )
      return val;
  }
  return target
...
const acts = [
  ...
  },
  {
    num: 5,
    name: "Act V: Results",
    narrative: `${tasksDone}/${tasksTotal} tasks completed. LLM coordination at scale.`,
  },
];

[0, 5, 15, 30, 40].forEach((b, i) =>
  events.push({ tick: Math.round(b * SCALE), type: "act_change", data: { act: i } }),
);
...
for (const d of decisions) {
  ...
  if (!seen.has(from)) {
    seen.add(from);
    events.push({ tick: t, type: "node_status", data: { agent: from, status: "working" } });
  }
  if (d.action === "delegate") {
    events.push({
      ...
      data: {
        from,
        to: resolveTarget(d.target),
        text: `📋 ${d.agentName} → ${d.target}: ${d.message.slice(0, 120)}`,
      },
    });
    ...
    events.push({
      tick: t,
      type: "message",
      data: { from, text: `🔨 ${d.message.slice(0, 120)}` },
    });
...
for (let t = 0; t <= MAX_TICK; t += 8) {
  ...
  events.push({
    ...
    data: {
      stats: {
        kitchenRate: Math.floor(p * 20),
        queueSize: Math.max(0, Math.floor(p * tasksTotal) - Math.floor(p * tasksDone)),
        deliveryRate: Math.floor(p * 12),
        revenue: Math.floor(p * 50000),
        margin: +(3.5 + p * 0.5).toFixed(1),
        budgetUsed: Math.floor(p * 95),
        pattiesProduced: Math.floor(p * tasksTotal),
...
events.push({
  ...
  data: {
    stats: {
      ...
      deliveryRate: 0,
      queueSize: 0,
      pattiesProduced: tasksTotal,
      pattiesDelivered: tasksDone,
      revenue: 50000,
      margin: 3.8,
      budgetUsed: 96,
    },
...
events.push({
  ...
  data: {
    from: "mr-krabs",
    text: `🎉 LLM Simulation complete! ${tasksDone}/${tasksTotal} tasks. ${totalDecisions} decisions by ${model}.`,
  },
});
for (const id of seen)
  events.push({ tick: MAX_TICK, type: "node_status", data: { agent: id, status: "idle" } });

events.sort((a, b) => a.tick - b.tick || a.type.localeCompare(b.type));
const filtered: Ev[] = [];
const cap = new Map<number, number>();
for (const e of events) {
  if (e.type === "message" || e.type === "delegation") {
    const c = cap.get(e.tick) || 0;
    if (c >= 3) continue;
    cap.set(e.tick, c + 1);
  }
  filtered.push(e);
