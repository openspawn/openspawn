...
const minimalProviders: ProviderConfig[] = [
  {
    ...
    models: [{ id: 'small-model', name: 'Small', costPer1kInput: 0, costPer1kOutput: 0, contextWindow: 4096, capabilities: ['chat'], maxTokens: 1024 }],
    enabled: true,
    priority: 1,
  },
  {
    ...
    models: [{ id: 'big-model', name: 'Big', costPer1kInput: 5.0, costPer1kOutput: 10.0, contextWindow: 128000, capabilities: ['chat', 'function-calling'], maxTokens: 8192 }],
    enabled: true,
    priority: 2,
  },
];
  ...
  it('L9+ executives route to premium provider', () => {
    ...
    const providers: ProviderConfig[] = [
      { ...minimalProviders[0] },
      {
        ...
        models: [{ id: 'anthropic/claude-3.5-sonnet', name: 'Claude', costPer1kInput: 3, costPer1kOutput: 15, contextWindow: 200000, capabilities: ['chat'], maxTokens: 8192 }],
        enabled: true,
        priority: 3,
      },
    ];
  ...
  it('L7-8 leads route to groq 70b when available', () => {
    const providers: ProviderConfig[] = [
      { ...minimalProviders[0] },
      {
        ...
        models: [
          { id: 'llama-3.1-8b-instant', name: '8B', costPer1kInput: 0.05, costPer1kOutput: 0.08, contextWindow: 131072, capabilities: ['chat'], maxTokens: 8192 },
          { id: 'llama-3.1-70b-versatile', name: '70B', costPer1kInput: 0.59, costPer1kOutput: 0.79, contextWindow: 131072, capabilities: ['chat'], maxTokens: 8192 },
        ],
...
describe('ModelRouter – no providers enabled', () => {
  it('returns none/none when all providers disabled', () => {
    const providers = minimalProviders.map(p => ({ ...p, enabled: false, models: [...p.models] }));
    const router = new ModelRouter(providers);
...
describe('ModelRouter – metrics', () => {
  ...
  it('tracks local routed count', () => {
    const providers: ProviderConfig[] = [{
      ...
      name: 'Ollama',
      baseUrl: 'http://localhost:11434',
      models: [{ id: 'qwen2.5:7b', name: 'Qwen', costPer1kInput: 0, costPer1kOutput: 0, contextWindow: 32768, capabilities: ['chat'], maxTokens: 4096 }],
      enabled: true,
      priority: 1,
    }];
    const router = new ModelRouter(providers);
...
describe('ModelRouter – route decision fields', () => {
  it('includes agentId and taskType', () => {
    const router = new ModelRouter(minimalProviders);
    const decision = router.route(req({ agentId: 'agent-42', agentLevel: 3, taskType: 'coding', preferLocal: true }));
