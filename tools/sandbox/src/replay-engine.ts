  ...
  for (const d of dirs) {
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          `  ⚠️ Failed to parse scenario ${file}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
  }
...
export function matchScenario(order: string, scenarios: ReplayScenario[]): ReplayScenario | null {
  ...
  if (bestScenario && bestScore > 0.3) {
    console.log(
      `[Replay] Matched "${bestScenario.name}" (score: ${bestScore.toFixed(2)}) for order: "${order.slice(0, 60)}..."`,
    );
    return bestScenario;
  }

  console.log(
    `[Replay] No scenario matched for order: "${order.slice(0, 60)}..." (best score: ${bestScore.toFixed(2)})`,
  );
  return null;
}
...
export class ReplaySimulation extends DeterministicSimulation {
  ...
  constructor(
    ...
    if (replayFile) {
      ...
      if (!existsSync(filePath)) {
        ...
        if (scenario) {
          console.log(
            `   • "${scenario.name}" (${scenario.decisions.length} decisions, ${scenario.metadata.ticks} ticks, model: ${scenario.metadata.model})`,
          );
        } else {
      ...
      for (const s of this.scenarios) {
        console.log(
          `   • "${s.name}" (${s.decisions.length} decisions, ${s.metadata.ticks} ticks)`,
        );
      }
    }
  }
  ...
  processOrder(order: string): void {
    ...
    if (matched) {
      ...
      console.log(
        `[Replay] Queued ${matched.decisions.length} decisions across ticks: ${tickList.join(", ")}`,
      );
    ...
    if (this.replayActive) {
      ...
      if (decisions) {
        ...
        for (const decision of decisions) {
          ...
          if (!agent) {
            console.log(
              `  🔁 ⚠️ Agent "${decision.agentName}" not found, skipping replay decision`,
            );
            continue;
          }
          ...
          console.log(
            `  🔁 ${agent.name}: ${decision.action} → ${decision.target} | ${decision.message.slice(0, 60)}`,
          );
