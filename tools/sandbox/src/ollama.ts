...
export function buildContext(
  ...
  lines.push(
    `Tasks completed: ${agent.stats.tasksCompleted} | Credits: ${agent.stats.creditsEarned - agent.stats.creditsSpent}`,
  );
  ...
  if (blockedTasks.length > 0) {
    ...
    for (const t of blockedTasks) {
      ...
      const fromName = lastEsc
        ...
        : "unknown";
      lines.push(
        `- [${t.id}] "${t.title}" — BLOCKED by ${fromName}: ${lastEsc?.body || t.blockedReason || "unknown reason"} [${lastEsc?.reason || ""}]`,
      );
    }
  }
  ...
  if (myMessages.length > 0) {
    ...
    for (const m of myMessages) {
      ...
      lines.push(
        `- ${prefix} [${m.type}] From ${fromAgent?.name || m.from} (${age}s ago): "${content}"`,
      );
    }
    lines.push(
  ...
  if (agent.level >= 7 && !hasReports && hasUnassigned) {
    ...
    lines.push(
      `Example: {"action":"spawn_agent","name":"Tech Lead","domain":"Engineering","role":"talent","reason":"Need engineering lead to handle tasks"}`,
    );
    ...
    lines.push(
      `Example: {"action":"spawn_agent","name":"Engineering Lead","domain":"Engineering","role":"lead","reason":"Need lead to handle engineering tasks"}`,
    );
