...
export class DecisionRecorder {
  ...
  toMarkdown(): string {
    const maxTick = this.entries.length > 0 ? Math.max(...this.entries.map((e) => e.tick)) : 0;
    ...
    const avgLatency =
      withDuration.length > 0
        ? Math.round(
            withDuration.reduce((s, e) => s + (e.durationMs || 0), 0) / withDuration.length,
          )
        : 0;
...
- **Actions:** ${actionLines}
- **Avg latency:** ${avgLatency}ms per LLM call${
      this.summary
        ? `
