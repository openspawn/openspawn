...
export class ModelRouter {
  ...
  route(req: RouteRequest): RouteDecision {
    ...
    if (req.agentLevel >= 9) {
      ...
      if (or) {
        ...
        selectedModel =
          selectedProvider.models.find((m) => m.id.includes("70b")) || selectedProvider.models[0];
      ...
      if (groq && !this.isRateLimited(groq)) {
        ...
        selectedModel =
          selectedProvider.models[selectedProvider.models.length - 1] || selectedProvider.models[0];
    ...
    const estimatedCost =
      selectedModel.costPer1kInput * inputTokens + selectedModel.costPer1kOutput * outputTokens;
  ...
  private recordDecision(
    ...
    this.metrics.requestsByProvider[provider.id] =
      (this.metrics.requestsByProvider[provider.id] || 0) + 1;
    this.metrics.costByProvider[provider.id] =
      (this.metrics.costByProvider[provider.id] || 0) + decision.estimatedCost;
    ...
    this.metrics.avgLatencyByProvider[provider.id] =
      prevCount > 0
  ...
  simulateFailure(providerId: string): void {
    this.metrics.failuresByProvider[providerId] =
      (this.metrics.failuresByProvider[providerId] || 0) + 1;
  }
}
