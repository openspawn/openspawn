...
describe('DeterministicSimulation', () => {
  describe('initialization', () => {
    it('starts with only the COO active', () => {
      const agents = [makeCOO(), makeLead('lead-1', 'Lead', 'coo'), makeWorker('worker-1', 'Worker', 'lead-1')];
    ...
    it('falls back to all agents when no COO exists', () => {
      const agents = [makeWorker('w1', 'Worker 1', undefined), makeWorker('w2', 'Worker 2', undefined)];
      const sim = new DeterministicSimulation(agents, DEFAULT_CONFIG, false, undefined, TEST_SEED);
      expect(sim.agents).toHaveLength(2);
    });

    it('starts with zero tasks', () => {
      const sim = new DeterministicSimulation([makeCOO()], DEFAULT_CONFIG, false, undefined, TEST_SEED);
...
`;
      ...
      const sim = new DeterministicSimulation(parsed.agents, DEFAULT_CONFIG, false, parsed, TEST_SEED);
