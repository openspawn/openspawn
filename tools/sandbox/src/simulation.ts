...
export class Simulation {
  ...
  constructor(
    ...
    this.log(
      `   ${agents.length} agents | ${this.tasks.length} seed tasks | model: ${config.model}`,
    );
  ...
  onEvent(callback: (event: SandboxEvent) => void): () => void {
    ...
    return () => {
      this.sseListeners = this.sseListeners.filter((l) => l !== callback);
    };
  }
    ...
    switch (action.action) {
      case "delegate": {
        ...
        if (task && target) {
          ...
          if (fuzzyTask && target) {
            ...
            this.logAgent(
              agent,
              `📋 Delegated "${fuzzyTask.title}" → ${target.name} (task fuzzy): ${reason}`,
            );
          } else {
            this.logAgent(
              agent,
              `⚠ Failed to delegate: task=${action.taskId} target=${action.targetAgentId}`,
            );
          }
        } else {
      ...
      case "work": {
        ...
        if (task) {
          ...
          this.logAgent(
            agent,
            `🔨 Working on "${task.title}" → ${task.status}: ${action.result}`,
            task.id,
          );
        }
        break;
      ...
      case "escalate": {
        ...
        if (task && parent) {
          ...
          this.logAgent(
            agent,
            `⬆️ Escalated "${task.title}" to ${parent.name}: [${reason}] ${body}`,
          );
        }
        break;
      }
      ...
      case "spawn_agent": {
        ...
        const candidate =
          ...
          notYetHired.find(
            (r) => r.role === requestedRole || r.domain?.toLowerCase().includes(requestedDomain),
          );
        ...
        if (candidate) {
          ...
          candidate.systemPrompt =
            candidate.systemPrompt || `Hired by ${agent.name}. ${action.reason || ""}`;
          this.agents.push(candidate);
          this.logAgent(
            agent,
            `🐣 Hired "${candidate.name}" from roster (L${candidate.level} ${candidate.domain} ${candidate.role}): ${action.reason}`,
          );
          break;
        }
        ...
        this.logAgent(
          agent,
          `🐣 Spawned "${newName}" (L${newLevel} ${newDomain} ${newRole}): ${action.reason}`,
        );
        break;
      }
  ...
  async runTick(): Promise<void> {
    ...
    const actingAgents = this.agents.filter((agent) => {
      ...
      if (agent.trigger === "event-driven") {
        ...
        if (hasInbox) console.log(`    ✉️ ${agent.name} inbox: ${agent.inbox.length} → WILL ACT`);
          ...
          console.log(`    💤 ${agent.name} (event-driven) — inbox empty, sleeping`);
        return hasInbox;
      }
    ...
    const maxPerTick =
      process.env.GROQ_API_KEY || process.env.OPENROUTER_API_KEY ? 3 : actingAgents.length;
    const agentsThisTick = actingAgents.slice(0, maxPerTick);
    if (actingAgents.length > maxPerTick) {
      console.log(
        `  ${actingAgents.length} agents want to act, capping to ${maxPerTick} this tick\n`,
      );
    } else {
  ...
  async restart(mode: "organic" | "full" = "organic"): Promise<void> {
    ...
    this.tick = 0;
    this.log(
      `   ${this.agents.length} agent(s) | ${this.tasks.length} seed tasks | model: ${this.config.model}`,
    );
  }
  ...
  printSummary(): void {
    ...
    if (top.length > 0) {
      ...
      for (const a of top) {
        console.log(
          `  ${a.name} (L${a.level}): ${a.stats.tasksCompleted} tasks, ${a.stats.messagesSent} messages`,
        );
      }
    }
    console.log(`${"─".repeat(60)}`);
