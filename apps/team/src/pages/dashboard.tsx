...
function AgentCard({
  ...
  const completion = agent.tasksCompleted > 0 ? agent.tasksSuccessful / agent.tasksCompleted : 0;
  const credit = agent.budgetPeriodLimit ? agent.budgetPeriodSpent / agent.budgetPeriodLimit : 0;
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="flex h-2.5 rounded-full overflow-hidden bg-white/5">
              ...
              {stats.wip > 0 && (
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${(stats.wip / stats.total) * 100}%` }}
                  transition={{ duration: 0.8, ease: "easeOut", delay: 0.1 }}
                />
              )}
              {stats.pending > 0 && (
                <motion.div
                  className="bg-amber-500/40 rounded-r-full"
                  initial={{ width: 0 }}
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-[11px] text-muted-foreground">
              ...
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-cyan-500 inline-block" /> In Progress (
                {stats.wip})
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-amber-500/40 inline-block" /> Pending (
                {stats.pending})
              </span>
            </div>
          </CardContent>
