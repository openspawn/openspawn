          ...
          {COLUMNS.map((col) => (
            <div key={col.id} className={`rounded-xl border p-4 space-y-3 ${col.color}`}>
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-white/10 px-2 py-0.5 text-xs text-white/60">{byColumn[col.id].length}</span>
              </div>
              {byColumn[col.id].length === 0 ? (
                ...
                byColumn[col.id].map((task) => (
                  <button key={task.id} onClick={() => openTaskPanel(task.id)}
                    ...
                    <p className="text-sm font-medium text-white leading-snug group-hover:text-white">{task.title}</p>
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${priorityColor(task.priority)}`}>{task.priority}</span>
                      {task.assignee && <span className="text-[10px] text-white/40">{task.assignee.name}</span>}
                    </div>
                  </button>
                ))
              )}
