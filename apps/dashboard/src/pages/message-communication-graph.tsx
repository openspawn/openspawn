...
export function CommunicationGraph({ messages, agents }: CommunicationGraphProps) {
  ...
  useSandboxSSE(useCallback((event: SandboxSSEEvent) => {
    ...
    if (event.agentId) {
      const relevantEdge = edges.find((e) => e.source === event.agentId || e.target === event.agentId);
      if (relevantEdge) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
      ...
      {selectedEdge && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="md:w-72">
          <Card>
                  ...
                  {selectedMessages.slice(0, 20).map((msg) => (
                    <div key={msg.id} className="p-2 rounded-lg bg-muted/50">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs font-medium">{msg.fromAgent?.name || 'Unknown'}</span>
