...
export function AgentAvatar({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("avatar-style-changed", handleStyleChange as EventListener);
  }, []);
  ...
  const avatar = resolvedAvatarUrl ? (
    ...
    <Avatar
      className={cn(sizeConfig.class, showRing && !presenceStatus && "ring-2", className)}
      style={{
        backgroundColor: bgColor,
        ...(showRing && !presenceStatus
          ? ({ "--tw-ring-color": accentColor } as React.CSSProperties)
    ...
    <Avatar
      ...
      style={
        showRing && !presenceStatus
          ? ({ "--tw-ring-color": levelColor } as React.CSSProperties)
          : undefined
      }
    >
  ...
  if (presenceStatus) {
    return (
      <div className="relative inline-flex" data-testid="agent-avatar">
        <PresenceGlow status={presenceStatus}>{avatar}</PresenceGlow>
        <StatusDot
          status={presenceStatus}
          size="sm"
          className="absolute -bottom-0.5 -right-0.5 z-10"
