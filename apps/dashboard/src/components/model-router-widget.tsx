...
export function ModelRouterWidget() {
  ...
  useEffect(() => {
    if (fetchedDecisions) setDecisions(fetchedDecisions);
  }, [fetchedDecisions]);
    ...
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-500/10 text-amber-400/70 border border-amber-500/15">
            sim
          </span>
      ...
      <div className="space-y-1">
        {Object.entries(totalByProvider)
          .sort((a, b) => b[1] - a[1])
          .map(([id, count]) => (
      ...
      <div className="flex justify-between text-xs">
        <span className="text-white/40">
          Cost:{" "}
          <span className={metrics.totalCost === 0 ? "text-emerald-400" : "text-amber-400"}>
