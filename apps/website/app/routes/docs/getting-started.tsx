...
export function GettingStarted() {
  ...
  return (
    <DocsLayout>
      ...
      <p className="mb-8 text-lg text-slate-400">
        Get OpenSpawn running in 2 minutes with the live demo or locally via CLI.
      </p>

      <div className="mb-8 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-4 py-3 text-sm text-cyan-300">
        💡 See it live at{" "}
        <a href="https://bikinibottom.ai/app/" target="_blank" rel="noopener" className="underline">
          bikinibottom.ai/app
        </a>{" "}
        — 22 SpongeBob agents running right now. No setup required.
      </div>
      ...
      <p className="text-slate-400">
        This creates <code className="rounded bg-white/10 px-1.5 py-0.5 text-cyan-400">ORG.md</code> and{" "}
        <code className="rounded bg-white/10 px-1.5 py-0.5 text-cyan-400">openspawn.config.json</code>.
      </p>
      ...
      <p className="text-slate-400">
        Open <code className="rounded bg-white/10 px-1.5 py-0.5 text-cyan-400">http://localhost:3333</code> — your dashboard is live.
      </p>
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'`}</CodeBlock>
      ...
      <p className="text-slate-400">
        Edit <code className="rounded bg-white/10 px-1.5 py-0.5 text-cyan-400">ORG.md</code> to define your agents,
        teams, and hierarchy. Changes are picked up on restart.
      </p>
    </DocsLayout>
  );
}
