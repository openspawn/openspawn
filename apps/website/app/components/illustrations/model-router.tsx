...
export function ModelRouterIllustration({ className = "" }: { className?: string }) {
  ...
  return (
    ...
    >
      {/* ── Left: Task input ─────────────────────────────────────────────── */}
      <rect x="2" y="41" width="22" height="18" rx="3" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.10)" strokeWidth="1" />
      <text x="13" y="48" textAnchor="middle" fontSize="5" fill="#94a3b8" fontFamily="monospace">TASK</text>
      ...
      <text x="12" y="55.5" textAnchor="middle" fontSize="3.5" fill="#22d3ee" fontFamily="monospace">L7</text>

      {/* ── Center: Router box ───────────────────────────────────────────── */}
      {/* Glow */}
      <circle cx="46" cy="50" r="16" fill="rgba(34,211,238,0.06)" />
      {/* Router card */}
      <rect x="33" y="36" width="26" height="28" rx="5" fill="rgba(18,45,77,0.90)" stroke="#22d3ee" strokeWidth="1.5" />
      ...
      {/* Router label */}
      <text x="46" y="48" textAnchor="middle" fontSize="5.5" fill="#22d3ee" fontFamily="monospace" fontWeight="800">
        ROUTE
      </text>
      ...
      <line x1="24" y1="50" x2="32" y2="50" stroke="#22d3ee" strokeWidth="1" strokeDasharray="1.5,1.5" />
      <path d="M30,47 L33,50 L30,53" fill="none" stroke="#22d3ee" strokeWidth="1" strokeLinejoin="round" />
      ...
      {providers.map((p) => {
        ...
        return (
          <g key={p.label}>
            ...
            />
            {/* Provider pill */}
            <rect x="71" y={p.y} width="26" height="14" rx="3" fill={`${p.color}18`} stroke={p.color} strokeWidth="1" />
            <text
              x="84"
              y={p.y + 6.5}
              textAnchor="middle"
      ...
      {/* Router output arrow */}
      <path d="M57,47 L60,50 L57,53" fill="none" stroke="#22d3ee" strokeWidth="1" strokeLinejoin="round" />

      {/* ── Cost savings badge ────────────────────────────────────────────── */}
      <rect x="2" y="78" width="30" height="14" rx="3" fill="rgba(52,211,153,0.08)" stroke="rgba(52,211,153,0.20)" strokeWidth="0.8" />
      <text x="17" y="84.5" textAnchor="middle" fontSize="5" fill="#34d399" fontFamily="monospace" fontWeight="700">local-first</text>
      <text x="17" y="89.5" textAnchor="middle" fontSize="4" fill="#34d399" fontFamily="monospace" opacity="0.7">saves ~75% cost</text>
