...
Values are stored in `openspawn.json` under the `alignment` key:
...
To change values: edit `openspawn.json` directly, or re-run `openspawn init` and select different values in the wizard.
...
>
> Yes. Edit the `alignment.values` array in `openspawn.json`. The template just sets the initial selection.
...
Open `openspawn.json` and modify the `alignment.values` array:
