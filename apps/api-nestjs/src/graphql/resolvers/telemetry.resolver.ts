...
export class TelemetryResolver {
  ...
  async tracesByOperation(@Args("operationName") operationName: string): Promise<TraceSpanType[]> {
  ...
  async metricsByName(@Args("name") name: string): Promise<MetricPointType[]> {
