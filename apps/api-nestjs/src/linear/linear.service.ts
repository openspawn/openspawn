...
export class LinearService implements IntegrationProvider {
  ...
  constructor(
    ...
    private readonly connectionRepo: Repository<LinearConnection>,
  ...
  async create(orgId: string, dto: CreateLinearConnectionDto): Promise<LinearConnection> {
    ...
    const syncConfig: LinearSyncConfig = {
      inbound: {
        ...
        createTaskOnComment:
          dto.createTaskOnComment ?? DEFAULT_SYNC_CONFIG.inbound.createTaskOnComment,
  ...
  async update(
    ...
    if (dto.createTaskOnComment !== undefined)
      conn.syncConfig.inbound.createTaskOnComment = dto.createTaskOnComment;
