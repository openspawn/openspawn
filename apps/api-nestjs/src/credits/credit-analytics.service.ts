...
export class CreditAnalyticsService {
   ...
   */
  async getSpendingTrends(orgId: string, days = 30, agentId?: string): Promise<SpendingTrend[]> {
  ...
  async getAgentSpendingSummary(orgId: string): Promise<AgentSpendingSummary[]> {
    ...
    for (const agent of agents) {
      const stats = await this.transactionRepository
        .createQueryBuilder("tx")
        .select("COUNT(*)", "count")
        .addSelect("SUM(CASE WHEN tx.type = 'CREDIT' THEN tx.amount ELSE 0 END)", "earned")
        .addSelect("SUM(CASE WHEN tx.type = 'DEBIT' THEN tx.amount ELSE 0 END)", "spent")
  ...
  async getAlerts(orgId: string): Promise<CreditAlert[]> {
    ...
    for (const agent of agents) {
      ...
      if (agent.budgetPeriodLimit && agent.budgetPeriodSpent > agent.budgetPeriodLimit) {
        alerts.push({
          agentId: agent.id,
          agentName: agent.name,
