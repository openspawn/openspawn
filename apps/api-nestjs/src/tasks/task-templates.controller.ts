...
export class TaskTemplatesController {
  ...
  @Post()
  async createTemplate(@CurrentAgent() agent: AuthenticatedAgent, @Body() dto: CreateTemplateDto) {
    const template = await this.templatesService.createTemplate(agent.orgId, agent.id, dto);
  ...
  async instantiateTemplate(
    ...
    const tasks = await this.templatesService.instantiateTemplate(agent.orgId, agent.id, dto);
    return { data: tasks };
  }
}
