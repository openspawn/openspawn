...
describe("TasksService - create / findAll / findOne / approve / assign", () => {
  ...
  describe("create", () => {
    ...
    it("should set creatorId to actorId", async () => {
      ...
      expect(taskRepo.create).toHaveBeenCalledWith(expect.objectContaining({ creatorId: actorId }));
    });
  ...
  describe("assign", () => {
    ...
    it("should throw NotFoundException when task does not exist", async () => {
      ...
      await expect(service.assign(orgId, actorId, "nonexistent", "agent-1")).rejects.toThrow();
    });
