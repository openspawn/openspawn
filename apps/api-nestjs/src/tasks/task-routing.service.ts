...
export class TaskRoutingService {
  ...
  async findCandidates(
    ...
    const taskCountMap = new Map(taskCounts.map((r) => [r.assigneeId, parseInt(r.count, 10)]));
    ...
    for (const [agentId, agentCaps] of agentCapMap) {
      ...
      const score = Math.round(coverageScore + proficiencyScore + levelScore + workloadScore);
  ...
  async suggestAgents(
    ...
    for (const [agentId, caps] of agentCapMap) {
      ...
      const missingCapabilities = normalized.filter((c) => !matchedCapabilities.includes(c));

      const coveragePercent = Math.round((matchedCapabilities.length / normalized.length) * 100);

      const proficiencySum = caps.reduce((sum, c) => sum + PROFICIENCY_WEIGHTS[c.proficiency], 0);
