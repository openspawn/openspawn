...
export class ConsensusController {
  ...
  async createConsensusRequest(
    ...
    const request = await this.consensusService.createRequest(agent.orgId, agent.id, dto);
    return { data: request };
  }
  ...
  async getVotableConsensus(@CurrentAgent() agent: AuthenticatedAgent) {
    const requests = await this.consensusService.getVotableRequests(agent.orgId, agent.id);
    return { data: requests };
  }
