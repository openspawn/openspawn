...
import { CurrentAgent, Roles, type AuthenticatedAgent, Public } from "../auth";
...
import {
  AgentBudgetService,
  type SetBudgetDto,
  type TransferCreditsDto,
} from "./agent-budget.service";
import {
  AgentCapabilitiesService,
  type AddCapabilityDto,
  type UpdateCapabilityDto,
} from "./agent-capabilities.service";
...
export class AgentsController {
   ...
   */
  @Post(":id/activate")
  async activate(@CurrentAgent() actor: AuthenticatedAgent, @Param("id") id: string) {
   ...
   */
  @Get(":id/budget")
  async getBudgetStatus(@CurrentAgent() actor: AuthenticatedAgent, @Param("id") id: string) {
  ...
  async canSpend(
    ...
    const result = await this.budgetService.canSpend(actor.orgId, id, parseInt(amount, 10));
    return { data: result };
  }
  ...
  async findBestMatch(
    ...
    const match = await this.capabilitiesService.findBestMatch(actor.orgId, capList, {
      minProficiency: minProficiency as Proficiency | undefined,
      onlyActive: true,
    });
   ...
   */
  @Get(":id/capabilities")
  async getAgentCapabilities(@CurrentAgent() actor: AuthenticatedAgent, @Param("id") id: string) {
    const capabilities = await this.capabilitiesService.getAgentCapabilities(actor.orgId, id);
    return { data: capabilities };
  }
  ...
  async addCapability(
    ...
    const capability = await this.capabilitiesService.addCapability(actor.orgId, actor.id, id, dto);
  ...
  async removeCapability(
    ...
    await this.capabilitiesService.removeCapability(actor.orgId, actor.id, capabilityId);
   ...
   */
  @Get(":id/reputation")
  async getReputation(@CurrentAgent() actor: AuthenticatedAgent, @Param("id") agentId: string) {
