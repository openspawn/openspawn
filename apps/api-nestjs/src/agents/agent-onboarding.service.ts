...
export class AgentOnboardingService {
  ...
  async canSpawnChild(
    parentId: string,
  ): Promise<{ canSpawn: boolean; reason?: string; current: number; max: number }> {
   ...
   */
  async activateAgent(orgId: string, actorId: string, agentId: string): Promise<Agent> {
    ...
    if (agent.status !== AgentStatus.PENDING) {
      throw new BadRequestException(`Agent is not pending activation (status: ${agent.status})`);
    }
    ...
    if (!isParent && !isL10) {
      throw new ForbiddenException("Only the parent agent or L10 can activate this agent");
    }
  ...
  async rejectAgent(
    ...
    if (!isParent && !isL10) {
      throw new ForbiddenException("Only the parent agent or L10 can reject this agent");
    }
