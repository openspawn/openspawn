...
const PROMOTION_REQUIREMENTS: Record<number, { minTrustScore: number; minTasksCompleted: number }> =
  {
...
export class TrustService {
  ...
  async applyQualityPenalty(params: {
    ...
    return this.recordEvent({
      ...
      reason: params.reason,
      customImpact: params.penaltyAmount ? -Math.abs(params.penaltyAmount) : undefined,
    });
  }
   ...
   */
  async demoteAgent(params: { agentId: string; demotedBy: string; reason: string }): Promise<void> {
    const agent = await this.agentRepo.findOneByOrFail({ id: params.agentId });
      ...
      if (requirements) {
        promotionProgress = {
          ...
          tasksProgress: Math.min(
            100,
            Math.round((agent.tasksCompleted / requirements.minTasksCompleted) * 100),
          ),
        };
      }
    }
