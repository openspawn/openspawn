...
export class AgentCapabilitiesService {
  ...
  async addCapability(
    ...
    if (!isSelf && !isParent && !isL10) {
      throw new ForbiddenException("Only the agent, their parent, or L10 can modify capabilities");
    }
   ...
   */
  async removeCapability(orgId: string, actorId: string, capabilityId: string): Promise<void> {
    const cap = await this.capabilityRepository.findOne({
      where: { id: capabilityId, orgId },
    });
  ...
  async findBestMatch(
    ...
    const matches = await this.findAgentsWithCapabilities(orgId, requiredCapabilities, options);
    ...
    for (const match of matches) {
      ...
      if (existing) {
        ...
        agentScores.set(match.agentId, {
          ...
          capabilities: [{ capability: match.capability, proficiency: match.proficiency }],
    ...
    for (const [agentId, data] of agentScores) {
      ...
      if (
        ...
        (coveragePercent === best.coveragePercent && data.totalScore > best.totalScore)
      ) {
        best = {
          agentId,
