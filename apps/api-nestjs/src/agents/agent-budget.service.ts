...
export class AgentBudgetService {
  ...
  async setBudget(
    ...
    if (!isParent && !isL10) {
      throw new ForbiddenException("Only parent agent or L10 can set budgets");
    }
  ...
  async transferCredits(
    ...
    if (!isChild && !isSibling && !isL10) {
      throw new ForbiddenException("Can only transfer credits to children or sibling agents");
    }
