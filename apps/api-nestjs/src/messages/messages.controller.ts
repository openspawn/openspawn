...
export class MessagesController {
  ...
  async sendDirectMessage(
    ...
    const message = await this.directMessagesService.sendDirectMessage(agent.orgId, agent.id, dto);
    return { data: message };
  }
  ...
  async getConversations(@CurrentAgent() agent: AuthenticatedAgent) {
    const conversations = await this.directMessagesService.getConversations(agent.orgId, agent.id);
  ...
  async getUnreadCount(@CurrentAgent() agent: AuthenticatedAgent) {
    const count = await this.directMessagesService.getUnreadCount(agent.orgId, agent.id);
    return { data: { count } };
  }
  ...
  async markAsRead(
    ...
    const count = await this.directMessagesService.markAsRead(agent.orgId, agent.id, otherAgentId);
