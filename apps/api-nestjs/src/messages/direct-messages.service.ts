...
export class DirectMessagesService {
   ...
   */
  async getOrCreateDMChannel(orgId: string, agentId1: string, agentId2: string): Promise<Channel> {
   ...
   */
  async markAsRead(orgId: string, currentAgentId: string, otherAgentId: string): Promise<number> {
    const channel = await this.getOrCreateDMChannel(orgId, currentAgentId, otherAgentId);
  ...
  async getConversations(orgId: string, agentId: string): Promise<Conversation[]> {
    ...
    for (const channel of channels) {
      const participants = (channel.metadata as Record<string, unknown>)?.["participants"] as
        | string[]
        | undefined;
