...
describe("DirectMessagesService", () => {
  ...
  beforeEach(() => {
    ...
    channelRepo = {
      ...
      save: vi
        .fn()
        .mockImplementation((data) => Promise.resolve({ id: "channel-new", ...data } as Channel)),
    ...
    messageRepo = {
      ...
      save: vi
        .fn()
        .mockImplementation((data) => Promise.resolve({ id: "msg-new", ...data } as Message)),
      createQueryBuilder: vi.fn().mockReturnValue({
        where: vi.fn().mockReturnThis(),
        andWhere: vi.fn().mockReturnThis(),
  ...
  describe("sendDirectMessage", () => {
    it("should create a message with recipientId", async () => {
      ...
      (agentRepo.findOne as Mock).mockResolvedValueOnce(fromAgent).mockResolvedValueOnce(toAgent);
    ...
    it("should throw NotFoundException when recipient agent not found", async () => {
      ...
      (agentRepo.findOne as Mock).mockResolvedValueOnce(fromAgent).mockResolvedValueOnce(null);
  ...
  describe("getDirectMessages", () => {
    it("should return conversation between two agents", async () => {
      ...
      (channelRepo.findOne as Mock).mockResolvedValue(channel);
      (agentRepo.findOne as Mock).mockResolvedValueOnce(agent1).mockResolvedValueOnce(agent2);
    ...
    it("should throw NotFoundException when agent not found", async () => {
      ...
      await expect(service.getDirectMessages(orgId, "agent-1", "agent-2")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
