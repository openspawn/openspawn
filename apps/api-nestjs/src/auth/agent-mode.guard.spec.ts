...
describe("AgentModeGuard", () => {
  ...
  describe("multiple modes allowed", () => {
    ...
    it("should allow any mode when all three are required", () => {
      ...
      expect(guard.canActivate(createMockExecutionContext({ mode: AgentMode.WORKER }))).toBe(true);
      expect(guard.canActivate(createMockExecutionContext({ mode: AgentMode.ORCHESTRATOR }))).toBe(
        true,
      );
      expect(guard.canActivate(createMockExecutionContext({ mode: AgentMode.OBSERVER }))).toBe(
        true,
      );
    });
  });
