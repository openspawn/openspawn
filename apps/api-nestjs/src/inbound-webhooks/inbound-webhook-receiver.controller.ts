...
export class InboundWebhookReceiverController {
  ...
  private verifySignature(
    ...
    const isValid = this.inboundWebhooksService.verifySignature(secret, payload, signatureHeader);
  ...
  async createTask(
    ...
    this.logger.log(`Creating task via webhook: ${webhookKey.name} (${webhookKey.id})`);
  ...
  async createTasks(
    ...
    const tasks = await Promise.all(
      dto.tasks.map(async (taskDto) => {
        ...
        const assigneeId = taskDto.assigneeId || webhookKey.defaultAgentId || undefined;
