...
const illustrations: Record<EmptyStateVariant, (compact?: boolean) => React.ReactNode> = {
  agents: (compact) => (
    <svg viewBox="0 0 120 100" className="w-full h-full text-cyan-500">
      ...
      <path
        d="M15 95 Q18 80 20 70 Q22 60 25 70 Q28 80 30 95"
        stroke="currentColor"
        strokeWidth={1.5}
        fill="none"
        opacity={0.25}
      />
      <path
        d="M90 95 Q93 82 96 75 Q99 82 102 95"
        stroke="currentColor"
        strokeWidth={1.5}
        fill="none"
        opacity={0.2}
    ...
    <svg viewBox="0 0 120 100" className="w-full h-full text-cyan-500">
      ...
      >
        <circle
          ...
          fillOpacity={0.1}
        />
        <text x="60" y="56" textAnchor="middle" fill="currentColor" fontSize="16" opacity={0.5}>
          ¢
        </text>
      </motion.g>
    </svg>
...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <p className={`text-muted-foreground max-w-sm mt-1 ${compact ? "text-xs" : "text-sm"}`}>
        {description}
      </p>
      {ctaLabel && onCta && (
        <Button
