...
export function ProtocolActivity() {
  ...
  useEffect(() => {
    ...
    source.onmessage = (e) => {
      try {
        ...
        const isA2A =
          event.type?.startsWith("a2a") ||
          event.type === "human_order" ||
  ...
  return (
    <div className="flex items-center gap-3 text-xs">
      <div
        className={`flex items-center gap-1.5 text-cyan-400 transition-all ${a2aPulse ? "scale-110" : ""}`}
      >
      ...
      <div
        className={`flex items-center gap-1.5 text-violet-400 transition-all ${mcpPulse ? "scale-110" : ""}`}
      >
        <div className="relative">
