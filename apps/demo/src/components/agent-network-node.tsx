      ...
      >
        ...
        {isActive && !isMobileOrTouch && (
          <motion.div
            ...
            transition={{ repeat: Infinity, duration: activityLevel === 'hot' ? 1 : 2, ease: "easeInOut" }}
        ...
        >
          ...
          <motion.div
            ...
            transition={{ repeat: isActive ? Infinity : 0, duration: activityLevel === 'hot' ? 1.5 : 2 }}
              ...
              return (
                <div className="relative mb-0.5" style={{ width: size, height: size }}>
                  <svg className="absolute inset-0" width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                    ...
                    <motion.circle cx={cx} cy={cx} r={r1} fill="none" stroke={col(health.completionRate)} strokeWidth={sw} strokeLinecap="butt" strokeDasharray={c1} initial={{ strokeDashoffset: c1 }} animate={{ strokeDashoffset: o1 }} transition={{ duration: 0.8, ease: 'easeOut' }} transform={`rotate(-90 ${cx} ${cx})`} />
                    <circle cx={cx} cy={cx} r={r2} fill="none" stroke="white" strokeOpacity={0.08} strokeWidth={sw} />
                    <motion.circle cx={cx} cy={cx} r={r2} fill="none" stroke={col(health.creditUsage)} strokeWidth={sw} strokeLinecap="butt" strokeDasharray={c2} initial={{ strokeDashoffset: c2 }} animate={{ strokeDashoffset: o2 }} transition={{ duration: 0.8, ease: 'easeOut', delay: 0.1 }} transform={`rotate(-90 ${cx} ${cx})`} />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="rounded-full overflow-hidden" style={{ width: avatarSize, height: avatarSize }}>
                      {nodeData.avatarUrl ? (
                        ...
                        <span className="w-full h-full inline-flex items-center justify-center text-lg leading-none select-none" style={{ backgroundColor: darkenForBackground(avatarColor) }}>{avatarEmoji}</span>
                      )}
                    </div>
                  </div>
                </div>
