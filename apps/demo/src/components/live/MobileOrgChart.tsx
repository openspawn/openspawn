...
const DEPARTMENTS: DeptDef[] = [
  ...
  },
  {
    ...
    memberIds: ['pearl-krabs', 'perch-perkins', 'barnacle-boy', 'larry-the-lobster', 'bubble-bass', 'dennis', 'flying-dutchman', 'fred-1', 'fred-2', 'fred-3'],
  },
  {
    id: 'finance',
    label: 'Finance',
...
function DeptCard({ dept, nodeStates, spawnedAgents, expanded, onToggle }: DeptCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {vpAgent?.avatarUrl ? (
            ...
            <span className="flex items-center justify-center w-full h-full text-2xl">{vpAgent?.emoji ?? '🐟'}</span>
          )}
          {/* Queue badge */}
          {vpState.queueBadge != null && vpState.queueBadge > 0 && (
            <div
              className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1"
            >
              {vpState.queueBadge > 999 ? `${(vpState.queueBadge / 1000).toFixed(1)}k` : vpState.queueBadge}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-sm truncate" style={{ color: '#B8E4F7', fontFamily: '"Baloo 2", cursive' }}>
            {vpAgent?.name ?? dept.vpId}
          </div>
          <div className="text-xs" style={{ color: 'rgba(184,228,247,0.5)', fontFamily: 'Nunito, sans-serif' }}>
            {dept.label}
          </div>
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: vpColor }} />
            <span className="text-[10px] font-semibold" style={{ color: vpColor, fontFamily: 'Nunito, sans-serif' }}>
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className="text-[10px]" style={{ color: 'rgba(184,228,247,0.4)', fontFamily: 'Nunito, sans-serif' }}>
      ...
      >
        <div className="px-3 pb-3 space-y-1.5 border-t" style={{ borderColor: 'rgba(74,174,217,0.1)' }}>
          <div className="pt-2 text-[10px] uppercase tracking-wider" style={{ color: 'rgba(184,228,247,0.3)', fontFamily: 'Nunito, sans-serif' }}>
            Team
          </div>
          ...
          {kitchenPool.length > 0 && (
            ...
            >
              <div className="text-[10px] font-bold mb-1" style={{ color: '#F4C542', fontFamily: '"Baloo 2", cursive' }}>
                🍳 Grill Station Pool
              </div>
              <div className="text-xs" style={{ color: '#B8E4F7', fontFamily: 'Nunito, sans-serif' }}>
...
export function MobileOrgChart({ nodeStates, spawnedAgents }: MobileOrgChartProps) {
  ...
  return (
    <div className="flex flex-col gap-2 p-3 overflow-y-auto" style={{ background: 'linear-gradient(180deg, #062A45 0%, #030E1A 100%)' }}>
      {/* CEO Card */}
      <div
      ...
      >
        ...
        <div>
          <div className="font-bold text-base" style={{ color: '#F4C542', fontFamily: '"Baloo 2", cursive' }}>
            {ceoAgent?.name ?? 'Mr. Krabs'}
          </div>
          <div className="text-xs" style={{ color: 'rgba(184,228,247,0.5)', fontFamily: 'Nunito, sans-serif' }}>CEO · The Krusty Krab</div>
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: ceoColor }} />
            <span className="text-[10px] font-semibold" style={{ color: ceoColor, fontFamily: 'Nunito, sans-serif' }}>
