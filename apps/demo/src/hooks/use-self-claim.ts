...
export interface UseSelfClaimOptions {
  agentId: string;
...
export function useSelfClaim({ agentId, onSuccess, onError }: UseSelfClaimOptions) {
  ...
  const mut = useMutation({
    mutationFn: async (): Promise<ClaimResult> => {
      if (isDemo && engine) {
        ...
        return claimed
          ? {
              success: true,
              message: `Claimed ${claimed.identifier}`,
              task: {
                id: claimed.id,
                identifier: claimed.identifier,
                title: claimed.title,
                status: "IN_PROGRESS",
                priority: claimed.priority.toUpperCase(),
                assigneeId: agentId,
    ...
    onSuccess: (result: ClaimResult) => {
      qc.invalidateQueries({ queryKey: ["claimableTaskCount"] });
  ...
  const claimNextTask = useCallback(async () => {
    setClaiming(true);
    try {
      return await mut.mutateAsync();
    } finally {
      setClaiming(false);
    }
  }, [mut]);
  ...
  return {
    ...
    claimNextTask,
    isClaiming: claiming || mut.isPending,
    lastClaimResult: mut.data,
    claimError: mut.error,
