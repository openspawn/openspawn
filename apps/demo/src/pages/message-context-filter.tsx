...
export function ContextLinkedMessages({ messages, agents, onViewThread }: ContextLinkedMessagesProps) {
              ...
              >
                ...
                <div className="relative">
                  <Button
                    ...
                    size="sm"
                    onClick={() => { setShowAgentDropdown(!showAgentDropdown); setShowTaskDropdown(false); }}
                    ...
                    {showAgentDropdown && (
                      ...
                      >
                        <ScrollArea className="max-h-[280px]">
                          <div className="p-1">
                            <Button variant={selectedAgent === null ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start text-xs h-8" onClick={() => { setSelectedAgent(null); setShowAgentDropdown(false); }}>
                              All Agents
                            </Button>
                            {agents.map((agent) => (
                              <Button key={agent.id} variant={selectedAgent === agent.id ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start gap-2 text-xs h-8" onClick={() => { setSelectedAgent(agent.id); setShowAgentDropdown(false); }}>
                ...
                {tasks.length > 0 && (
                  <div className="relative">
                    <Button
                      ...
                      size="sm"
                      onClick={() => { setShowTaskDropdown(!showTaskDropdown); setShowAgentDropdown(false); }}
                      className="h-7 px-2 text-xs gap-1.5"
                    >
                      {selectedTask ? <>📋 <span className="max-w-[100px] truncate">{selectedTask}</span></> : <>📋 Task</>}
                      {showTaskDropdown ? '▲' : '▼'}
                    </Button>
                    <AnimatePresence>
                      {showTaskDropdown && (
                        ...
                        >
                          <ScrollArea className="max-h-[280px]">
                            <div className="p-1">
                              <Button variant={selectedTask === null ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start text-xs h-8" onClick={() => { setSelectedTask(null); setShowTaskDropdown(false); }}>
                                All Tasks
                              </Button>
                              {tasks.map((task) => (
                                <Button key={task} variant={selectedTask === task ? 'secondary' : 'ghost'} size="sm" className="w-full justify-start text-xs h-8" onClick={() => { setSelectedTask(task); setShowTaskDropdown(false); }}>
                                  📋 {task}
                                </Button>
                              ))}
                            </div>
                ...
                {hasFilters && (
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}>
                    <Button variant="ghost" size="sm" onClick={() => { setSelectedAgent(null); setSelectedTask(null); setTeamFilter('all'); }} className="h-7 px-2 text-xs text-destructive hover:text-destructive">
                      ✕ Clear
                    </Button>
                  </motion.div>
                )}
                        ...
                        <div className="flex gap-1 md:gap-2 mt-2 flex-wrap items-center">
                          ...
                          {msg.taskRef && (
                            <Badge
                              ...
                              onClick={(e) => { e.stopPropagation(); setSelectedTask(msg.taskRef!); }}
                            >
                              🔗 {msg.taskRef}
                            </Badge>
                          )}
