...
export function MessagesPage() {
  ...
  const threadMessages = useMemo(() => {
    ...
    return messages.filter(
      (m) => [m.fromAgentId, m.toAgentId].sort().join("::") === threadConvoKey,
    );
    ...
    <div className="p-3 md:p-6 space-y-4 md:space-y-6">
      ...
      <Tabs defaultValue="feed" className="w-full">
        <TabsList className="grid w-full grid-cols-4 mb-4 md:mb-6 h-9 md:h-10">
          <TabsTrigger value="graph" className="text-xs md:text-sm px-1 md:px-3">
            🕸️ <span className="hidden sm:inline ml-1">Graph</span>
          </TabsTrigger>
          <TabsTrigger value="feed" className="text-xs md:text-sm px-1 md:px-3">
            📡 <span className="hidden sm:inline ml-1">Feed</span>
          </TabsTrigger>
          <TabsTrigger value="cards" className="text-xs md:text-sm px-1 md:px-3">
            💬 <span className="hidden sm:inline ml-1">Cards</span>
          </TabsTrigger>
          <TabsTrigger value="context" className="text-xs md:text-sm px-1 md:px-3">
        ...
        <TabsContent value="graph">
          <Card className="p-3 md:p-4">
            <p className="text-xs md:text-sm text-muted-foreground mb-3 md:mb-4">
              <strong>Communication Graph:</strong> Tap edges to see conversations. Edges pulse
              green when messages flow.
            </p>
        ...
        <TabsContent value="feed">
          <Card className="p-3 md:p-4">
            <p className="text-xs md:text-sm text-muted-foreground mb-3 md:mb-4">
              <strong>Mission Control:</strong> Real-time stream of all agent communications. Click
              "View thread" to see the full conversation.
            </p>
        ...
        <TabsContent value="context">
          <Card className="p-3 md:p-4 relative overflow-visible">
            <p className="text-xs md:text-sm text-muted-foreground mb-3 md:mb-4">
              <strong>Context-Linked:</strong> Filter by agent, task, or team to see related
              discussions.
            </p>
      ...
      <AnimatePresence>
        {threadConvoKey && threadMessages.length > 0 && (
          <ThreadView messages={threadMessages} onClose={() => setThreadConvoKey(null)} />
        )}
      </AnimatePresence>
    </div>
  );
