...
test.describe("Demo Mode - Initialization", () => {
  test("should load demo mode and show initial event", async ({ page, context }) => {
    ...
    await expect(page.getByText("Simulation started", { exact: false })).toBeVisible({
      timeout: 5000,
    });
  });
...
test.describe("Demo Mode - Simulation", () => {
  ...
  test("should update agents page when new agents spawn", async ({ page }) => {
    ...
    const initialCount =
      (await page.locator('[data-testid="agent-card"]').count()) ||
      (await page
        .locator(".grid > div")
        .filter({ has: page.getByText("Agent") })
        .count());
...
test.describe("Demo Mode - Data Consistency", () => {
  test("should show same agent count across Agents page and Dashboard", async ({ page }) => {
    ...
    const dashboardActiveAgents = await page
      .getByText("Active Agents")
      .locator("..")
      .locator("div.text-2xl")
      .textContent();
    ...
    const agentsPageActive = await page
      .getByText("Active")
      .locator("..")
      .locator("div.text-2xl")
      .textContent();
  ...
  test("should show same event count across pages after navigation", async ({ page }) => {
    ...
    const eventsCountAfter = await page
      .getByText("Total Events")
      .locator("..")
      .locator("div.text-2xl")
      .textContent();
