...
test.describe("Network page responsive tests", () => {
  for (const viewport of viewports) {
    test(`renders correctly at ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      page,
    }) => {
  ...
  test("stats bar is present", async ({ page }) => {
    ...
    await expect(
      page.locator(".rounded-2xl, .rounded-full").filter({ hasText: "Active" }),
    ).toBeVisible();
    await expect(
      page.locator(".rounded-2xl, .rounded-full").filter({ hasText: "Pending" }),
    ).toBeVisible();
  });
