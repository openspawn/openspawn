  ...
  for (const scenario of SCENARIOS) {
    test(`can switch to ${scenario} scenario`, async ({ page }) => {
      ...
      const scenarioBtn = page
        .locator(`button:has-text("${scenario.charAt(0).toUpperCase() + scenario.slice(1)}")`)
        .first();
...
test.describe("Navigation", () => {
  ...
  test("navigation works on mobile", async ({ page }) => {
    ...
    for (const pageInfo of PAGES.slice(0, 4)) {
      // Test first 4 pages
      await page.click(`a[href*="${pageInfo.path}"]`);
      await expect(page.locator(pageInfo.selector)).toBeVisible({ timeout: 5000 });
    }
...
test.describe("Tasks Page", () => {
  ...
  test("can switch between Kanban and List view", async ({ page }) => {
    ...
    await expect(
      page.locator('button[role="tab"][aria-selected="true"]:has-text("List")'),
    ).toBeVisible();
  });
...
test.describe("Demo Controls", () => {
  ...
  test("can play and pause simulation", async ({ page }) => {
    ...
    const playPauseBtn = page
      .locator("button")
      .filter({ has: page.locator("svg.lucide-play, svg.lucide-pause") })
      .first();
