  ...
  switch (sub) {
    case 'list': {
      ...
      for (const t of tasks) {
        console.log(`  ${t.id}  [${t.status.padEnd(11)}]  ${t.assignee ?? '(unassigned)'}  ${t.description}`);
      }
      return;
    }
    case 'create': {
      const desc = rest.join(' ');
      if (!desc) { console.error('Usage: openspawn task create <description>'); return; }
    ...
    case 'next': {
      ...
      if (!task) { console.log('No open tasks.'); return; }
      console.log(`Claimed ${task.id}: ${task.description}`);
      return;
    }
    case 'done': {
      const id = rest[0];
      if (!id) { console.error('Usage: openspawn task done <id>'); return; }
