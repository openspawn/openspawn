...
export function hireCommand(args: string[], ctx: { dir: string; orgFile?: string }) {
  ...
  if (!name) { console.error('Usage: openspawn hire <name> [--level N] [--model M] [--parent P]'); return; }

  const orgPath = ctx.orgFile ?? join(ctx.dir, 'ORG.md');
  let org: ParsedOrg;
  if (existsSync(orgPath)) { org = parseOrgMd(orgPath); } else { org = { name: 'Unnamed Org', agents: [], culture: {}, policies: {} }; }
  ...
  org.agents.push({ id, name, role: 'worker', level, domain: 'general', parentId: parent, model, status: 'active' });
