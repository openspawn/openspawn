    ...
    for (const dept of structureSection.children) {
      ...
      if (isCLevel || dept.children.length === 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          agents.push({
            id: agentId, name: agentName, role: inferred.role,
            level: deptLevel, domain, parentId,
            model: deptMeta['model'], status: 'active',
          });
      ...
      for (let ri = 0; ri < dept.children.length; ri++) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const agentId = count > 1 ? `${id}-${i + 1}` : id;
          agents.push({
            id: agentId, name: agentName, role: subInferred.role,
            level: subLevel, domain, parentId,
            model: subMeta['model'], status: 'active',
          });
        }
      }
    }
...
export function generateOrgMd(org: ParsedOrg): string {
  ...
  if (org.culture.preset || org.culture.escalationVelocity) {
    ...
    if (org.culture.escalationVelocity) lines.push(`- **Escalation:** ${org.culture.escalationVelocity}`);
    if (org.culture.progressFrequency) lines.push(`- **Progress Updates:** ${org.culture.progressFrequency}`);
    if (org.culture.ackRequired !== undefined) lines.push(`- **Ack Required:** ${org.culture.ackRequired ? 'yes' : 'no'}`);
    lines.push('');
  }

  if (Object.keys(org.policies).length > 0) {
    lines.push('## Policies', '');
    if (org.policies.perAgentBudget) lines.push(`- **Per-Agent Limit:** $${org.policies.perAgentBudget}`);
    if (org.policies.alertThreshold) lines.push(`- **Alert Threshold:** ${org.policies.alertThreshold}%`);
    lines.push('');
  }
