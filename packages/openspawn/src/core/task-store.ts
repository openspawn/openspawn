...
export function createTask(dir: string, description: string, opts?: { assignee?: string; delegatedBy?: string }): Task {
...
export function listTasks(dir: string, filter?: { assignee?: string; status?: TaskStatus }): Task[] {
  const store = loadStore(dir);
...
export function updateTask(dir: string, taskId: string, updates: { status?: TaskStatus; pr?: number; assignee?: string }): Task | null {
  const store = loadStore(dir);
