...
export function registerTools(server: McpServer, dir: string, orgFile?: string): void {
  ...
  server.tool('org_read', 'Parse ORG.md and return the structured organization', {}, async () => {
    const path = orgPath();
    if (!existsSync(path)) return { content: [{ type: 'text', text: JSON.stringify({ error: 'ORG.md not found at ' + path }) }] };
    const org = parseOrgMd(path);
    return { content: [{ type: 'text', text: JSON.stringify(org, null, 2) }] };
  });

  // ── org_update ─────────────────────────────────────────────────────────
  server.tool('org_update', 'Update ORG.md (add/remove agents, update policies)', {
    ...
    if (params.action === 'add_agent' && params.agentName) {
      ...
      org.agents.push({
        id, name: params.agentName, role: 'worker',
        level: params.agentLevel ?? 4, domain: params.agentDomain ?? 'general',
        parentId: params.agentParent, model: params.agentModel, status: 'active',
      });
    } else if (params.action === 'remove_agent' && params.agentName) {
      const id = params.agentName.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      org.agents = org.agents.filter(a => a.id !== id && a.name.toLowerCase() !== params.agentName!.toLowerCase());
    ...
    return { content: [{ type: 'text', text: JSON.stringify({ ok: true, agents: org.agents.length }) }] };
  });

  // ── task_list ──────────────────────────────────────────────────────────
  server.tool('task_list', 'List tasks, optionally filtered', {
  ...
  // ── task_claim ─────────────────────────────────────────────────────────
  server.tool('task_claim', 'Claim the next available task (or a specific one)', {
    ...
    if (!task) return { content: [{ type: 'text', text: JSON.stringify({ error: 'No open task found' }) }] };
    return { content: [{ type: 'text', text: JSON.stringify(task, null, 2) }] };
  });

  // ── task_update ────────────────────────────────────────────────────────
  server.tool('task_update', 'Update task status/progress', {
    taskId: z.string(),
    status: z.enum(['open', 'claimed', 'in-progress', 'done', 'blocked']).optional(),
  ...
  // ── delegate ───────────────────────────────────────────────────────────
  server.tool('delegate', 'Delegate a task down the hierarchy', {
    description: z.string().describe('Task description'),
    ...
    if (existsSync(path)) {
      ...
      if (fromAgent && toAgent && toAgent.level >= fromAgent.level) {
        return { content: [{ type: 'text', text: JSON.stringify({ error: 'Cannot delegate to agent of equal or higher level' }) }] };
      }
    }
    const task = createTask(dir, params.description, { assignee: params.to, delegatedBy: params.from });
    return { content: [{ type: 'text', text: JSON.stringify(task, null, 2) }] };
  });

  // ── escalate ───────────────────────────────────────────────────────────
  server.tool('escalate', 'Escalate a task up the hierarchy', {
    taskId: z.string(),
    reason: z.string(),
    ...
    if (!task) return { content: [{ type: 'text', text: JSON.stringify({ error: 'Task not found' }) }] };
    return { content: [{ type: 'text', text: JSON.stringify({ ...task, escalationReason: params.reason, escalatedTo: parentId }) }] };
  });

  // ── hire ───────────────────────────────────────────────────────────────
  server.tool('hire', 'Add an agent to the organization', {
    name: z.string(),
    ...
    const path = orgPath();
    let org: ParsedOrg;
    if (existsSync(path)) { org = parseOrgMd(path); } else { org = { name: 'Unnamed Org', agents: [], culture: {}, policies: {} }; }
    const id = params.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const agent: Agent = {
      id, name: params.name, role: 'worker',
      level: params.level ?? 4, domain: params.domain ?? 'general',
      parentId: params.parent, model: params.model, status: 'active',
    };
  ...
  // ── fire ───────────────────────────────────────────────────────────────
  server.tool('fire', 'Remove an agent from the organization', {
    name: z.string(),
  }, async (params) => {
    const path = orgPath();
    if (!existsSync(path)) return { content: [{ type: 'text', text: JSON.stringify({ error: 'No ORG.md' }) }] };
    const org = parseOrgMd(path);
    const before = org.agents.length;
    const id = params.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    org.agents = org.agents.filter(a => a.id !== id && a.name.toLowerCase() !== params.name.toLowerCase());
    writeFileSync(path, generateOrgMd(org));
    return { content: [{ type: 'text', text: JSON.stringify({ ok: true, removed: before - org.agents.length }) }] };
  });

  // ── budget_check ───────────────────────────────────────────────────────
  server.tool('budget_check', 'Check an agent\'s remaining budget', {
    agentId: z.string(),
  }, async (params) => {
    const budget = getBudget(dir, params.agentId);
    if (!budget) return { content: [{ type: 'text', text: JSON.stringify({ error: 'No budget set', agentId: params.agentId }) }] };
  ...
  // ── budget_spend ───────────────────────────────────────────────────────
  server.tool('budget_spend', 'Record a spend against an agent\'s budget', {
    agentId: z.string(),
    amount: z.number(),
  ...
  // ── report ─────────────────────────────────────────────────────────────
  server.tool('report', 'Agent reports status or completion', {
    agentId: z.string(),
    ...
    if (params.taskId) {
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ agentId: params.agentId, status: params.status, task }) }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify({ agentId: params.agentId, status: params.status }) }] };
  });
}
