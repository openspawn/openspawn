...
async function handleAgentRegister(
  db: Db,
  params: { id: string; name: string; role?: string; level?: number },
) {
  try {
...
async function handleEscalate(
  db: Db,
  params: { from_agent: string; to_agent?: string; task_id?: string; reason: string },
) {
...
describe("escalations", () => {
  it("creates an escalation and lists it as open", async () => {
    const result = await handleEscalate(db, {
      from_agent: "agent-1",
...
describe("concurrent access patterns", () => {
  ...
  it("rapid sequential claims exhaust available tasks correctly", async () => {
    const ids = [createTask(db, { title: "Rapid 1" }), createTask(db, { title: "Rapid 2" })];
