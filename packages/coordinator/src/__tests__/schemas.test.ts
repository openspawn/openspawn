...
describe("TaskResultSchema — invalid inputs", () => {
  it("rejects an unknown type discriminant", () => {
    expect(() => TaskResultSchema.parse({ type: "unknown_type", value: 42 })).toThrow(ZodError);
  });

  it("rejects pr_merged with missing pr field", () => {
    expect(() => TaskResultSchema.parse({ type: "pr_merged", branch: "main" })).toThrow(ZodError);
  });

  it("rejects pr_merged when pr is a string instead of number", () => {
    expect(() => TaskResultSchema.parse({ type: "pr_merged", pr: "not-a-number" })).toThrow(
      ZodError,
    );
  });

  it("rejects file_created with missing paths", () => {
    expect(() => TaskResultSchema.parse({ type: "file_created", description: "oops" })).toThrow(
      ZodError,
    );
  });

  it("rejects file_created when paths is not an array", () => {
    expect(() => TaskResultSchema.parse({ type: "file_created", paths: "src/foo.ts" })).toThrow(
      ZodError,
    );
  });

  it("rejects docs_updated with missing files", () => {
    expect(() => TaskResultSchema.parse({ type: "docs_updated" })).toThrow(ZodError);
  });

  it("rejects config_changed with missing changes", () => {
    expect(() => TaskResultSchema.parse({ type: "config_changed" })).toThrow(ZodError);
  });

  it("rejects research_complete with missing findings", () => {
    expect(() => TaskResultSchema.parse({ type: "research_complete" })).toThrow(ZodError);
  });

  it("rejects error with missing reason", () => {
    expect(() => TaskResultSchema.parse({ type: "error", recoverable: true })).toThrow(ZodError);
  });
  ...
  it("rejects escalation with missing severity", () => {
    expect(() => TaskResultSchema.parse({ type: "escalation", issue: "Something bad" })).toThrow(
      ZodError,
    );
  });

  it("rejects freeform with missing text", () => {
    expect(() => TaskResultSchema.parse({ type: "freeform" })).toThrow(ZodError);
  });
