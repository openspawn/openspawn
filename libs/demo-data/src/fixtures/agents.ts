...
export const agents: DemoAgent[] = [
  ...
  // ═══════════ LEVEL 1-2 — PROBATION ═══════════
  {
    ...
    trustScore: 35,
    reputationLevel: ReputationLevel.PROBATION,
    tasksCompleted: 3,
    tasksSuccessful: 2,
...
function getReputationLevel(trustScore: number): ReputationLevel {
  if (trustScore >= 86) return ReputationLevel.ELITE;
  if (trustScore >= 71) return ReputationLevel.VETERAN;
  if (trustScore >= 41) return ReputationLevel.TRUSTED;
  if (trustScore >= 31) return ReputationLevel.PROBATION;
