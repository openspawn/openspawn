...
export interface TaskPanelTask {
  ...
  rejection?: { feedback?: string | null; rejectedAt?: string | null; rejectionCount?: number | null } | null;
}
...
function statusConfig(s: string): { icon: React.ReactNode; color: string; label: string; bg: string } {
  switch (s.toUpperCase()) {
    case "DONE": return { icon: <CheckCircle2 className="w-4 h-4" />, color: "text-emerald-400", label: "Done", bg: "bg-emerald-500/20 border-emerald-500/30" };
    case "IN_PROGRESS": return { icon: <Activity className="w-4 h-4" />, color: "text-cyan-400", label: "In Progress", bg: "bg-cyan-500/20 border-cyan-500/30" };
    case "BLOCKED": return { icon: <AlertTriangle className="w-4 h-4" />, color: "text-red-400", label: "Blocked", bg: "bg-red-500/20 border-red-500/30" };
    case "REVIEW": return { icon: <Clock className="w-4 h-4" />, color: "text-violet-400", label: "In Review", bg: "bg-violet-500/20 border-violet-500/30" };
    case "TODO": return { icon: <Clock className="w-4 h-4" />, color: "text-blue-400", label: "To Do", bg: "bg-blue-500/20 border-blue-500/30" };
    case "BACKLOG": return { icon: <Clock className="w-4 h-4" />, color: "text-white/40", label: "Backlog", bg: "bg-white/10 border-white/10" };
    case "CANCELLED": return { icon: <XCircle className="w-4 h-4" />, color: "text-red-400/60", label: "Cancelled", bg: "bg-red-500/10 border-red-500/20" };
    default: return { icon: <Clock className="w-4 h-4" />, color: "text-white/40", label: s, bg: "bg-white/10 border-white/10" };
  }
}

function priorityConfig(p: string): { color: string; icon: React.ReactNode } {
  switch (p.toUpperCase()) {
    case "URGENT": return { color: "bg-red-500/20 text-red-400 border-red-500/30", icon: <Flag className="w-3 h-3" /> };
    case "HIGH": return { color: "bg-amber-500/20 text-amber-400 border-amber-500/30", icon: <Flag className="w-3 h-3" /> };
    case "NORMAL": return { color: "bg-blue-500/20 text-blue-400 border-blue-500/30", icon: <Flag className="w-3 h-3" /> };
    default: return { color: "bg-white/10 text-white/40 border-white/10", icon: <Flag className="w-3 h-3" /> };
  }
}
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-0">
            ...
            {task.approvalRequired && !task.approvedAt && (
              <MetaRow icon={<AlertTriangle className="w-3.5 h-3.5 text-amber-400" />} label="Approval" value="Required" />
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {(isBlocked || isCancelled) && (
        <div className={cn("ml-2 text-[10px] font-medium px-1.5 py-0.5 rounded", isBlocked ? "bg-red-500/20 text-red-400" : "bg-white/10 text-white/40")}>
