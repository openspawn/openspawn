...
function mapAgent(raw: Record<string, unknown>): AgentFields {
  return {
    ...
    role: ((raw.role as string) ?? "") as AgentRole,
    mode: ((raw.mode as string) ?? "") as AgentMode,
    status: ((raw.status as string) ?? "") as AgentStatus,
    level: (raw.level as number) ?? 1,
    model: (raw.model as string) ?? "",
    currentBalance: (raw.current_balance as number) ?? (raw.currentBalance as number) ?? 0,
    budgetPeriodLimit:
      (raw.budget_period_limit as number | null) ??
      (raw.budgetPeriodLimit as number | null) ??
      null,
    budgetPeriodSpent:
      (raw.budget_period_spent as number) ?? (raw.budgetPeriodSpent as number) ?? 0,
    ...
    reputationLevel: ((raw.reputation_level as string) ??
      (raw.reputationLevel as string) ??
      "") as ReputationLevel,
    ...
    lastActivityAt:
      (raw.last_activity_at as string | null) ?? (raw.lastActivityAt as string | null) ?? null,
    lastPromotionAt:
      (raw.last_promotion_at as string | null) ?? (raw.lastPromotionAt as string | null) ?? null,
    lifetimeEarnings: (raw.lifetime_earnings as number) ?? (raw.lifetimeEarnings as number) ?? 0,
    defaultAutonomyLevel:
      (raw.default_autonomy_level as number) ?? (raw.defaultAutonomyLevel as number) ?? 0,
